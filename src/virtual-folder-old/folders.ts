...
export class Folders {
    ...
    _setObserver(
        ...
        return new Promise((resolve) => {
            const observerCB: MutationCallback = function (
                mutations,
                observer
            ) {
...
`
    ...
    _initFolders(node: HTMLElement): void {
        ...
        this.gasStaticRoot = new GasRoot(
            this.dom.gasFileList,
            this._saveStaticsFolder.bind(this)
        );
    ...
    _saveStaticsFolder(): void {
        ...
        this._timeOut_saveStaticFolders = setTimeout(() => {
            localStorage.setItem(
                `appScriptColor-static-Folders-${this._key}`,
                JSON.stringify(this.gasStaticRoot.getDeepToggleState())
            );
    ...
    private _loadVirtualFolder() {
        ...
        try {
            state = JSON.parse(
                localStorage.getItem(`appScriptColor-Folders-${this._key}`)
            );
        } catch (e) {}
    ...
    private _saveInfoShown(): void {
        ...
        this._timeOut_infoShown = setTimeout(() => {
            localStorage.setItem(
                `appScriptColor-infoShown`,
                JSON.stringify(getInfoShown())
            );
        }, 500) as unknown as number;
    }
    ...
    private _loadInfoShown(): void {
        ...
        try {
            infoShown = JSON.parse(
                localStorage.getItem(`appScriptColor-infoShown`)
            );
        } catch (e) {}
