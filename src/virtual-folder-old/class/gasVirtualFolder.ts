...
export class GasVirtualFolder extends GasFolder {
    ...
    toggleInfo(state?: boolean): void {
        ...
        this.dom.infoPopup.classList.toggle(
	...
	<div class="asc_old_info_popup ${
        !getInfoShown().vFolder ? "asc_old_info_popup-hide" : ""
    }">
        ...
        this.dom.infoPopup =
            this.dom.titleContainer.querySelector(`.asc_old_info_popup`);
        this.dom.infoIcon =
            this.dom.titleContainer.querySelector(`.asc_old_folder_info`);
    ...
    private _processVirtualList(virtualList: IVirtualList[]): void {
        ...
        virtualList.forEach((folder) => {
            let currentFolder =
                (this.getChild(folder.name) as GasFolder) ||
                this.addChild(new GasFolder(folder.name, this._saveCallback));

            // assign folder to fileName
            folder.files.forEach((file) =>
                this._vList.set(file, currentFolder)
            );
        });
    }
}
