...
export class GasFolder {
    ...
    removeChild(child: GasFile | GasFolder): void {
        ...
        child.dom?.main?.parentElement &&
            child.dom.main.parentElement === this.dom?.main &&
            this.dom.main.removeChild(child.dom.main);
    }
    ...
    sortAllChildren(): void {
        ...
        this.children.forEach(
            (item) => item instanceof GasFolder && item.sortAllChildren()
        );
    }
    ...
    assignDomChildren(deepAssign: boolean): void {
        ...
        this.children.forEach((item) => {
            ...
            deepAssign &&
                item instanceof GasFolder &&
                item.assignDomChildren(true);
        });
    }
    ...
    clearEmptyFolder(): void {
        ...
        this.children.forEach(
            (item) => item instanceof GasFolder && item.clearEmptyFolder()
        );
...
<div class="${CLASS_CHILDLIST}"></div>`;
        ...
        this.dom.titleContainer = this.dom.main.querySelector(
            `.${CLASS_TITLE_CONTAINER}`
        );
    ...
    private _sortChildren() {
        this.children = new Set(
            Array.from(this.children.values()).sort((a, b) => {
