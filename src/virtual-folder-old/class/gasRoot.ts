...
export class GasRoot extends GasFolder {
    ...
    setUpVirtualFolder(virtualFolders: IVirtualList[]): void {
        this.virtualFolder =
            (this.getChild(GasVirtualFolder.NAME) as GasVirtualFolder) ||
            this.addChild(
                new GasVirtualFolder(virtualFolders, this._saveCallback)
            );
    ...
    private _getGasItems(): IMutatedFile {
        ...
        for (
            let i = 0, numChildren = this.root.childNodes.length;
            i < numChildren;
            i++
        ) {
            /**
            ...
            if (node === this.dom.main || node.classList.contains(CLASS_FOLDER))
                continue;
    ...
    private _updateChildList(forceUpdate?: boolean): void {
        ...
        let { removed, added, renamed } = (!forceUpdate &&
            this._getGasItems()) || {
            renamed: new Map(),
            ...
            splitPath.forEach((name, i) => {
                ...
                if (i === splitPath.length - 1) {
                    ...
                    if (
                        this.virtualFolder &&
                        currentFolder === this &&
                        this.virtualFolder.is_vFile(file)
                    ) {
                ...
                currentFolder =
                    ...
                    currentFolder.addChild(
                        new GasFolder(name, this._saveCallback)
                    );
            });
        });
