...
class SettingsStorage {
	...
	setCustomThemes(customThemes: ICustomThemes): void {
		return this._setSetting('theme', 'custom', JSON.stringify(customThemes));
	}
	...
	getFolderStates(scriptKey: string): IFolderStateDictionary {
		try {
			return JSON.parse(this._getSetting('static', `Folders-${ scriptKey }`) || '{}');
		} catch (e) {
			return {};
		}
	}

	setFolderStates(scriptKey: string, folderStates: IFolderStateDictionary): void {
		return this._setSetting('static', `Folders-${ scriptKey }`, JSON.stringify(folderStates));
	}
	...
	private _buildSettingKey(store: string, settingName?: string): string {
		return `${ this._storagePrefix }-${ store }${ settingName ? `-${ settingName }` : '' }`;
	}

	private _setSetting(store: string, settingName: string | undefined, value: string) {
