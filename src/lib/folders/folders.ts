...
export class Folders {
    ...
    destroy() {
        ...
        this._eventSubFolderChangedAction &&
            this._domFileList?.removeEventListener(
                EVENT_SUB_FOLDER_CHANGED,
                this._eventSubFolderChangedAction
            );
    ...
    private _listenForDomUpdate(): void {
        this._onDomUpdated = ({ detail: { node } }) => {
            ...
            const domFileListContainer = node.querySelector(
                'div[jsslot] ul[role="listbox"]'
            ) as HTMLElement;
...
`
    ...
    private _cleanInsertedStyle() {
        const domLink = document.head.querySelector(
            `link[ref-class="${this._classRef}"]`
        );
        const domStyle = document.head.querySelector(
            `style[ref-class="${this._classRef}"]`
        );
    ...
    private _initFolders(domFileList: HTMLElement): void {
        ...
        this.gasStaticRoot = new GasRoot(domFileList, () =>
            this._saveStaticsFolder()
        );
    ...
    private _saveStaticsFolder(): void {
        ...
        this._timeOut_saveStaticFolders = window.setTimeout(() => {
            settingsService.setFolderStates(
                this._key,
                this.gasStaticRoot.getDeepToggleState()
            );
        }, 500);
    }
